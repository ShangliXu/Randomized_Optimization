...
def opt_prob_domains(prob_domains_name, problem_size):
    ...
    elif prob_domains_name == 'FlipFlop':
        problem = FlipFlopGenerator().generate(seed=123456, size = problem_size) 
    elif prob_domains_name == 'ContinuousPeaks':
        problem = ContinuousPeaksGenerator().generate(seed=123456, size = problem_size) 
    return problem
...
prob_domains_name_list = ['ContinuousPeaks']#['Nqueens', 'MaxKColor', 'TSP']
problem_size_map = {'ContinuousPeaks':[10, 20, 50]}#{'Nqueens':[8, 20], 'MaxKColor':[8, 20], 'TSP':[10, 20, 100]}#[8, 20, 50]
fitness_list_list = []
for prob_domains_name in prob_domains_name_list:
    ...
    for problem_size_ in problem_size_list:
        ...
        for algo_func, algo_kwargs, algo_name in zip(algo_func_list, kwargs_list, algo_names):
            ...
            run_stats_best_run, run_curve_best_run = best_run(algo_name, df_run_stats, df_run_curves, max_min = 'max')
